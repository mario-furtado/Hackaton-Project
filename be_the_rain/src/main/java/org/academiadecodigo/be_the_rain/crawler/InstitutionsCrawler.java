...
import org.academiadecodigo.be_the_rain.services.InstituService;
...
@Component
public class InstitutionsCrawler {
    ...
    public void init(String word, InstituService instituService){
            ...
            if (newUrl.startsWith("q?doi")) {
                ...
                for(Element text : texts){
                    if(text.attr("class").equals("CitationsyArchives_search_result") && !(visitedUrls.containsValue(text.text()))){
                        ...
                        institution.setLink(newUrl);
                        institution.setTitle("Institutions");
                        institution.setContent(text.text());
                        instituService.persist(institution);
                        break;
                    }
                }
                linkTitle(newUrl);
