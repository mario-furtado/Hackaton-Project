...
import org.academiadecodigo.be_the_rain.services.AntiVaxService;
...
public class VaxCrawler {
    ...
    public void init(String word, AntiVaxService antiVaxService){
            ...
            if (newUrl.startsWith("q?doi")) {
                ...
                for(Element text : texts){
                    if(text.attr("class").equals("CitationsyArchives_search_result") && !(visitedUrls.containsValue(text.text()))){
                        ...
                        AntiVax antiVax = new AntiVax();
                        antiVax.setLink(newUrl);
                        antiVax.setTitle("Anti-Vax");
                        antiVax.setContent(text.text());
                        antiVaxService.persist(antiVax);
                        break;
                    }
                }
                linkTitle(newUrl);
